...
public class Wrapper {
    ...
    private Map<String, Wrapper.DisallowedMods> disallowedMods;
    private List<PingWorker> workers = new ArrayList();
    ...
    public void init() {
        ...
        this.gsonNonPretty = (new GsonBuilder()).registerTypeAdapterFactory(new EnumAdapterFactory())
                .enableComplexMapKeySerialization().create();
        try {
    ...
    public String getPlayerUUID() {
        return Minecraft.getMinecraft().getSession() != null
                && Minecraft.getMinecraft().getSession().getPlayerID() != null
                ? Minecraft.getMinecraft().getSession().getPlayerID() : "Error";
    }
    ...
    public int sendHeartBeat(String ip, int port) {
        Wrapper.HbResponse wrapper$hbresponse = (Wrapper.HbResponse) getInstance().gsonNonPretty
                .fromJson(this.sendInitialHeartbeat(ip, port), Wrapper.HbResponse.class);
    ...
    public boolean tick() {
        ...
        Minecraft.getMinecraft().getRenderManager().debugBoundingBox = this.getActiveModProfile().getHitboxes()
                .isEnabled();
        ...
        try {
            ...
            if (!getInstance().checkVerify()) {
                JOptionPane.showMessageDialog((Component) null,
                        "Please use the Badlion launcher!  If you are seeing this error constantly, please fully restart the Badlion Client and launcher.",
                        "Badlion Client", 0);
                System.exit(0);
            }
        ...
        if (Minecraft.getMinecraft().displayWidth != this.lastDisplayX
                || Minecraft.getMinecraft().displayHeight != this.lastDisplayY) {
        ...
        if (Minecraft.getMinecraft().gameSettings.keyBindPickBlock.getKeyCode() != -100
                && Minecraft.getMinecraft().gameSettings.keyBindPickBlock.getKeyCode() != -99) {
        ...
        if (!this.connected && Minecraft.getMinecraft().getNetHandler() != null
    ...
    public Map<String, Wrapper.DisallowedMods> getDisallowedMods() {
    ...
    public class HbResponse {
        private Map<String, Wrapper.DisallowedMods> modsDisallowed;
        private int hb;
