...
import net.minecraft.client.gui.BackgroundPanorama;
...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        Gui.backgroundPanorama = new BackgroundPanorama(this);
        Wrapper wrapper = new Wrapper();
        wrapper.init();
        Wrapper.getInstance().getModProfileManager().getAccountManager().addSession(this.session);
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn)
    {
        ...
        if (Wrapper.getInstance().isPremium() && guiScreenIn instanceof GuiDisconnected)
        {
            Wrapper.getInstance().updateConnection(false);
        }

        if (this.currentScreen == null && Wrapper.getInstance().getModProfileManager().getActiveModProfile().getSlideoutAccess().getSlideoutInstance().isVisible())
        {
    ...
    private void runGameLoop() throws IOException
    {
        ...
        if (!Wrapper.getInstance().tick())
        {
            for (Object object : KeyBinding.getKeybindArray())
            {
                KeyBinding keybinding = (KeyBinding)object;
                Object object1 = KeyBinding.getHash().lookup(keybinding.getKeyCode());
                KeyBinding keybinding1 = (KeyBinding)object1;

                if (keybinding1 == null)
                {
                    KeyBinding.resetKeyBindingArrayAndHash();
                }
                else if (keybinding1.getKeyCode() != keybinding.getKeyCode())
                {
                    KeyBinding.resetKeyBindingArrayAndHash();
                }
            }

            for (int k = 0; k < this.timer.elapsedTicks; ++k)
            {
                this.runTick();
            }

            this.mcProfiler.endStartSection("preRenderErrors");
            long l = System.nanoTime() - j;
            ...
            this.field_181542_y.func_181747_a(i1 - this.field_181543_z);
            this.field_181543_z = i1;
    ...
    {
        ClickMouse clickmouse = new ClickMouse(0);
        Wrapper.getInstance().getActiveModProfile().passEvent(clickmouse);
    ...
    {
        Wrapper.getInstance().getCapeManager().tickCapes();
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        boolean flag1 = Wrapper.getInstance().isDebugEnabled();
                        ...
                        if (flag1 && k == 31 && Keyboard.isKeyDown(61))
                        {
                            this.refreshResources();
                        }
                        ...
                        if (flag1 && k == 20 && Keyboard.isKeyDown(61))
                        {
                            this.refreshResources();
                        }
                        ...
                        if (flag1 && k == 30 && Keyboard.isKeyDown(61))
                        {
                            this.renderGlobal.loadRenderers();
                        }
                        ...
                        {
                            Wrapper.getInstance().getActiveModProfile().getHitboxes().toggle();
                        }
            ...
            else
            {
                ...
                while (this.gameSettings.keyBindDrop.isPressed())
                {
                    ClickMouse clickmouse = new ClickMouse(1);
                    Wrapper.getInstance().getActiveModProfile().passEvent(clickmouse);
    ...
    {
        Wrapper.getInstance().getActiveModProfile().getNewMiniMap().clearMinimapChunks();

        if (worldClientIn == null)
        {
            Wrapper.getInstance().getCapeManager().getUserToCape().clear();
    ...
    {
        ClickMouse clickmouse = new ClickMouse(2);
        Wrapper.getInstance().getActiveModProfile().passEvent(clickmouse);
    ...
    public void setSession(Session p_setSession_1_)
    {
        this.session = p_setSession_1_;
    }
