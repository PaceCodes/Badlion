...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (this.showCrosshair())
        {
            ...
            if (!Wrapper.getInstance().getActiveModProfile().getCrosshair().isEnabled())
            {
        ...
        ModProfileManager.callRenderGame(this);
        GlStateManager.enableAlpha();
    }
    ...
    {
        net.badlion.client.mods.render.Scoreboard.renderScoreboard(p_180475_1_, p_180475_2_);
    }
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            for (int i6 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1; i6 >= 0; --i6)
            {
                ...
                if (Wrapper.getInstance().getLegacyAnimations().getHeartAnimation().equals(LegacyAnimations.AnimationMode.CURRENT) && flag)
                {
                    if (i6 * 2 + 1 < j)
                    {
