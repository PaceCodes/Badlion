...
public class GuiNewChat extends Gui
{
    ...
    public void drawChat(int p_146230_1_)
    {
        ...
        {
            Chat chat = Wrapper.getInstance().getActiveModProfile().getChat();
                    ...
                    if (chatline != null)
                    {
                        ...
                        if (j1 < 200 || flag)
                        {
                            ...
                            if (l1 > 3)
                            {
                                int i2 = 0;
                                int j2 = -i1 * 9;
                                drawRect(i2, j2 - 9, i2 + l + 4, j2, chat.isEnabled() ? chat.getChatBackgroundColor().getColorInt() : l1 / 2 << 24);
                                IChatComponent ichatcomponent = chatline.getChatComponent();
                                String s = Wrapper.getInstance().getActiveModProfile().getChat().getPrefix(chatline.getDate());

                                if (s != null)
                                {
                                    ichatcomponent = (new ChatComponentText(s)).appendSibling(ichatcomponent);
                                }

                                String s1 = ichatcomponent.getFormattedText();
                                GlStateManager.enableBlend();

                                if (chat.isEnabled() && !chat.isTextBackgroundShadow())
                                {
                                    this.mc.fontRendererObj.drawString(s1, i2, j2 - 8, 16777215 + (l1 << 24));
                                }
                                else
                                {
                                    this.mc.fontRendererObj.drawStringWithShadow(s1, (float)i2, (float)(j2 - 8), 16777215 + (l1 << 24));
                                }
    ...
    {
        ChatReceivedEvent chatreceivedevent = new ChatReceivedEvent(p_146234_1_, this.field_146253_i);
        Wrapper.getInstance().getActiveModProfile().passEvent(chatreceivedevent);

        if (!chatreceivedevent.isCancelled())
        {
            p_146234_1_ = chatreceivedevent.getComponent();
