...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    {
        if (Wrapper.getInstance().getActiveModProfile().getEnchantGlint().isEnabled())
        {
    ...
    public void renderItemModelForEntity(ItemStack stack, EntityLivingBase entityToRenderFor, ItemCameraTransforms.TransformType cameraTransformType)
    {
        if (stack != null && entityToRenderFor != null)
        {
            ...
            if (entityToRenderFor instanceof EntityPlayer)
            {
                ...
                if (item == Items.fishing_rod && Wrapper.getInstance().getLegacyAnimations().getItemHeld().equals(LegacyAnimations.AnimationMode.LEGACY))
                {
                    modelresourcelocation = new ModelResourceLocation("7_fishing_rod", "inventory");
                }
                ...
                if (item == Items.fishing_rod && entityplayer.fishEntity != null)
                {
                    ...
                    if (Wrapper.getInstance().getLegacyAnimations().getItemHeld().equals(LegacyAnimations.AnimationMode.LEGACY))
                    {
                        modelresourcelocation = new ModelResourceLocation("7_fishing_rod_cast", "inventory");
                    }
                }
