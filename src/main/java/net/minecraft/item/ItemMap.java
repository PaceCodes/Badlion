...
public class ItemMap extends ItemMapBase
{
    ...
    public void updateMiniMapData(World p_updateMiniMapData_1_, Entity p_updateMiniMapData_2_, MapData p_updateMiniMapData_3_)
    {
        if (p_updateMiniMapData_1_.provider.getDimensionId() == p_updateMiniMapData_3_.dimension && p_updateMiniMapData_2_ instanceof EntityPlayer)
        {
            int i = 1 << p_updateMiniMapData_3_.scale;
            int j = p_updateMiniMapData_3_.xCenter;
            int k = p_updateMiniMapData_3_.zCenter;
            int l = MathHelper.floor_double(p_updateMiniMapData_2_.posX - (double)j) / i + 64;
            int i1 = MathHelper.floor_double(p_updateMiniMapData_2_.posZ - (double)k) / i + 64;
            int j1 = 128 / i / 2;
            MapData.MapInfo mapdata$mapinfo = p_updateMiniMapData_3_.getMapInfo((EntityPlayer)p_updateMiniMapData_2_);
                ...
                if ((k1 & 15) == (mapdata$mapinfo.field_82569_d & 15) || flag)
                {
                    ...
                    for (int l1 = i1 - j1 - 1; l1 < i1 + j1; ++l1)
                    {
                        if (k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128)
                        {
                            ...
                            Chunk chunk = p_updateMiniMapData_1_.getChunkFromBlockCoords(new BlockPos(k2, 0, l2));
                            ...
                            if (!chunk.isEmpty())
                            {
                                ...
                                if (p_updateMiniMapData_1_.provider.getHasNoSky())
                                {
                                    int l3 = k2 + l2 * 231871;
                                ...
                                {
                                    byte b0 = p_updateMiniMapData_3_.colors[k1 + l1 * 128];
                                    byte b1 = (byte)(mapcolor.colorIndex * 4 + i5);

                                    if (b0 != b1)
                                    {
                                        p_updateMiniMapData_3_.colors[k1 + l1 * 128] = b1;
                                        p_updateMiniMapData_3_.updateMapData(k1, l1);
                                        flag = true;
                                    }
                                }
                            }
