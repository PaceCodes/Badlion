...
public abstract class World implements IBlockAccess
{
    ...
    {
        if (!Wrapper.getInstance().getActiveModProfile().getBetterframesConfig().isWeather())
        {
    ...
    {
        if (Wrapper.getInstance().getActiveModProfile().getTimeChanger().isTimeFrozen())
        {
            time = Wrapper.getInstance().getActiveModProfile().getTimeChanger().getCurrentWorldTimePreset();
        }
    ...
    {
        return this.worldInfo.getTerrainType() == WorldType.FLAT ? 0.0D : 0.0D;
    }
