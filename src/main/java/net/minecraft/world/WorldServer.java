...
public class WorldServer extends World implements IThreadListener
{
    ...
    public void tick()
    {
        ...
        if (this.getGameRules().getBoolean("doDaylightCycle"))
        {
            double d0 = Wrapper.getInstance().getActiveModProfile().getTimeChanger().getTimeMultiplier();

            if (d0 < 1.0D)
            {
                if ((double)(this.timeTicks++) > 1.0D / d0)
                {
            ...
            {
                for (this.remainder += d0; this.remainder >= 1.0D; --this.remainder)
                {
                    this.worldInfo.setWorldTime(this.worldInfo.getWorldTime() + 1L);
                }
            }
    ...
    protected void updateBlocks()
    {
        ...
        else
        {
            ...
            {
                int i = chunkcoordintpair1.chunkXPos * 16;
                int j = chunkcoordintpair1.chunkZPos * 16;
                this.theProfiler.startSection("getChunk");
                Chunk chunk = this.getChunkFromChunkCoords(chunkcoordintpair1.chunkXPos, chunkcoordintpair1.chunkZPos);
                this.playMoodSoundAndCheckLight(i, j, chunk);
                ...
                if (this.rand.nextInt(100000) == 0 && this.isRaining() && this.isThundering())
                {
                    ...
                    BlockPos blockpos = this.adjustPosToNearbyEntity(new BlockPos(i + (k & 15), 0, j + (k >> 8 & 15)));
                ...
                if (this.rand.nextInt(16) == 0)
                {
                    ...
                    BlockPos blockpos2 = this.getPrecipitationHeight(new BlockPos(i + (k2 & 15), 0, j + (k2 >> 8 & 15)));
                    ...
                    for (ExtendedBlockStorage extendedblockstorage : chunk.getBlockStorageArray())
                    {
                        if (extendedblockstorage != null && extendedblockstorage.getNeedsRandomTick())
                        {
                            for (int l = 0; l < l2; ++l)
                            {
                                ...
                                int l1 = i1 >> 16 & 15;
                                ++j2;
                                IBlockState iblockstate = extendedblockstorage.get(j1, l1, k1);
                                Block block = iblockstate.getBlock();

                                if (block.getTickRandomly())
                                {
                                    ++i2;
                                    block.randomTick(this, new BlockPos(j1 + i, l1 + extendedblockstorage.getYLocation(), k1 + j), iblockstate, this.rand);
                                }
                            }
                        }
                    }
