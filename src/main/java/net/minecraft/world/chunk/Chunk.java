...
public class Chunk
{
    ...
    protected void generateHeightMap()
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            ...
            {
                this.precipitationHeightMap[j + (i1 << 4)] = -999;

                for (int j1 = i + 16; j1 > 0; --j1)
                {
                    Block block = this.getBlock0(j, j1 - 1, i1);

                    if (block.getLightOpacity() != 0)
                    {
                        IBlockState iblockstate = this.getBlockState(new BlockPos(j, j1 - 1, i1));
                        ...
                        abyte[i1 << 4 | j] = (byte)(block.getMapColor(iblockstate).colorIndex * 4 + l);
                        this.heightMap[i1 << 4 | j] = j1;

                        if (j1 < this.heightMapMinimum)
                        {
                            this.heightMapMinimum = j1;
                        }
        ...
        Wrapper.getInstance().getActiveModProfile().getNewMiniMap().generateChunk(abyte, this.xPosition, this.zPosition);
